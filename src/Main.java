...
public class Main 
{
  ...
  // The time complexity is:  // nested loop
  // O(n)2
  ...
  // The time complexity is: loops n times
  // O(n)
  ...
  // assume that each String is bounded by a constant length
  // The time complexity is: not a nested loop
  // O(n)
  ...
  public static String emailLookupEfficient(HashMap<String, String> namesToEmails, String queryName) {
    for(String entry : namesToEmails.keySet())
    {
      if(entry.equals(queryName))
      {
  ...
  // What is the time complexity of this method?
  // assume that each String is bounded by a constant length
  // (assume the set and list have the same number of elements). like a nested for loop
  // o(n)2
  ...
  public static boolean hasCommonEfficient(HashSet<String> wordSet, ArrayList<String> wordList) 
  {
    for(String word : wordSet) 
    {
      if(wordSet.contains(word)) //iterate to iterate through hashset to improve the time complexity
      {
        return true;
      }
    }
  ...
  // HashMap, the key as stock or ticker and value as price 
